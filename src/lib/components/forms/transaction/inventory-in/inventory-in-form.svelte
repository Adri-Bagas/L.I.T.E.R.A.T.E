<script lang="ts">
	...
	import { InventoryInAdded, InventoryInSelection } from '$lib/scripts/stores/inventory-in-table';
	...
	import InventoryIn from '$lib/scripts/controllers/transactions/inventory-in';
	...
	InventoryInSelection.set(books);

	const tableSelection = createTable(InventoryInSelection, {
		page: addPagination({
            initialPageSize: 5
        }),
    ...
    async function stupidlyHandlingSubmit(e: any) {
		...
		let datas = await InventoryIn.store(t, $formData);
		WaitingSomething.set(false);
		if (datas.success == true) {
			goto('/dashboard/transaction/inventory-in');
		}
	}
