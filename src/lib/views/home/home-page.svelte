<script lang="ts">
    ...
    let showedData: BookSmallView[] = []

    onMount(() => {console.log(datas); showedData = datas})

    let sq: string;

    function handleInput() {
        let searchQuery = sq.toLowerCase();

        if (searchQuery === ""){
            showedData = datas
        }

        showedData = datas.filter(item => item.title.toLowerCase().includes(searchQuery))
    }
	...
	<div class="hidden flex-col pb-12 md:flex">
		<div class="flex-1 space-y-4 p-8 pt-6 mb-32">
			<div class="flex items-center justify-between space-y-2">
				<h2 class="text-4xl font-bold tracking-tight">Hello! Read More Please!</h2>
			</div>
            ...
            <div class="flex items-center justify-between space-y-2">
				<h2 class="text-2xl font-bold tracking-tight ml-10">All of our collection!</h2>
                <div class="flex w-1/4 items-center gap-2">
                    <Input class="w-full" placeholder="Search..." bind:value={sq} on:change={handleInput}/>
                    <Search />
                </div>
			</div>
