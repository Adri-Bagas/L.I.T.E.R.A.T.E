...
<div class="h-screen overflow-auto flex items-center justify-center">
    <div class="w-1/2">
        {#if !isLight}
            <img src={NightIMG} alt="book" class="w-1/2 h-screen bg-background object-cover object-top top-0 left-0 absolute brightness-50"/>
        {:else}
            <img src={DayIMG} alt="book" class="w-1/2 h-screen bg-background object-cover object-right top-0 left-0 absolute brightness-50"/>
        {/if}
        ...
        <div class="w-1/3 text-lg font-semibold tracking-tight flex items-stretch z-10 absolute bottom-14 left-4 italic text-white"><h2>"{data.quote}"</h2></div>
        <div class="w-1/3 text-lg font-semibold tracking-tight flex items-stretch z-10 absolute bottom-8 left-4 italic text-white"><h2>- {data.author}</h2></div>
    </div>
    <div class="h-full w-1/2 flex justify-center items-center">
        <div class="h-[600px] w-3/4 border flex flex-col items-center border-blue-500 shadow-2xl py-6 rounded-2xl justify-center">
            ...
            <div class="w-3/4 flex flex-col gap-y-4 mb-6">
                <LoginForm data={data.form}></LoginForm>
            </div>

            <div>
                <a on:click|preventDefault={reqForPass} href="/" class="hover:text-blue-500">Forgot password ?</a>
            </div>
